 ...
 * SpecularButton.ts
 *
 * Vanilla TS port of React Bits' SpecularButton component.
 * Creates a WebGL specular highlight effect on button edges using OGL.
 * Finds all [data-specular] elements and initializes them.
 */
...
interface SpecularButtonInstance {
  destroy: () => void;
}

function createSpecularButton(btn: HTMLElement): SpecularButtonInstance {
  ...
  const thickness = parseFloat(btn.dataset.sbThickness ?? '1');
  const speed = parseFloat(btn.dataset.sbSpeed ?? '0.35');
  const followMouse = btn.dataset.sbFollowMouse !== 'false';
  const proximity = parseFloat(btn.dataset.sbProximity ?? '250');
  const autoAnimate = btn.dataset.sbAutoAnimate === 'true';
