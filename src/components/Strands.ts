 ...
 * Strands.ts
 *
 * WebGL strands animation using OGL.
...
export function initStrands(options: StrandsOptions = {}): (() => void) | null {
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uStrandCount: { value: Math.min(opts.count, MAX_STRANDS) },
  ...
  const glassProgram = new Program(gl, {
    ...
    uniforms: {
      ...
      uResolution: { value: [container.offsetWidth, container.offsetHeight] },
      uRadius: { value: 0.46 * opts.glassSize },
      uRefraction: { value: opts.refraction },
  ...
  const update = (t: number) => {
    ...
    program.uniforms.uColors.value = buildPalette(opts.colors);
    program.uniforms.uColorCount.value = Math.min(opts.colors.length, MAX_COLORS);
    program.uniforms.uStrandCount.value = Math.min(Math.max(Math.round(opts.count), 1), MAX_STRANDS);
    program.uniforms.uSpeed.value = opts.speed;
    program.uniforms.uAmplitude.value = opts.amplitude;
    program.uniforms.uWaviness.value = opts.waviness;
    program.uniforms.uThickness.value = opts.thickness;
    program.uniforms.uGlow.value = opts.glow;
    program.uniforms.uTaper.value = opts.taper;
    program.uniforms.uSpread.value = opts.spread;
    program.uniforms.uHueShift.value = opts.hueShift;
    program.uniforms.uIntensity.value = opts.intensity;
    program.uniforms.uOpacity.value = opts.opacity;
    program.uniforms.uScale.value = opts.scale;
    program.uniforms.uSaturation.value = opts.saturation;

    if (opts.glass) {
      renderer.render({ scene: mesh, target: renderTarget });
      glassProgram.uniforms.uScene.value = renderTarget.texture;
      glassProgram.uniforms.uRefraction.value = opts.refraction;
      glassProgram.uniforms.uDispersion.value = opts.dispersion;
      glassProgram.uniforms.uRadius.value = 0.46 * opts.glassSize;
